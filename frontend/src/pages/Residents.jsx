...
export default function Residents() {
  ...
  const [restrForm, setRestrForm] = useState({ employee_id:'', restriction_type:'forbidden', reason:'' });
      ...
      {showForm && (
        <div className="card mb-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2"><label className="label">Notizen / Besonderheiten</label><textarea className="input" rows={2} value={form.notes} onChange={e => setForm(f=>({...f,notes:e.target.value}))} /></div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-2 mt-1">
                  {r.needs_1to1 && <span className="badge-warn">1:1 Begleitung</span>}
                  {r.needs_night_supervision && <span className="badge-warn">Nachtaufsicht</span>}
                </div>
                ...
                {detail.restrictions?.map((rr, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3 text-sm mb-2">
                      <span className={rr.restriction_type === 'forbidden' ? 'badge-error' : 'badge-ok'}>
                      ...
                      <button onClick={() => removeRestriction(r.id, rr.employee_id)} className="text-red-500 hover:text-red-700 ml-auto">Entfernen</button>
                    </div>
                  );
                })}
                <form onSubmit={addRestriction} className="flex gap-2 mt-3">
                  <select className="input flex-1" required value={restrForm.employee_id} onChange={e => setRestrForm(f=>({...f,employee_id:e.target.value}))}>
                    <option value="">Mitarbeiter auswählen...</option>
                    {employees.map(e => <option key={e.id} value={e.id}>{e.short_code} – {e.last_name}</option>)}
                  </select>
                  <select className="input w-32" value={restrForm.restriction_type} onChange={e => setRestrForm(f=>({...f,restriction_type:e.target.value}))}>
                    <option value="forbidden">Verboten</option>
                    <option value="required">Pflicht</option>
                  </select>
                  <input className="input flex-1" placeholder="Begründung..." value={restrForm.reason} onChange={e => setRestrForm(f=>({...f,reason:e.target.value}))} />
